...
class VRAE:
    ...
    def create_gradientfunctions(self, data):
        ...
        def encodingstep(x_t, h_t):
            return T.tanh(T.dot(self.params["W_xhe"], x_t) + T.dot(self.params["W_hhe"], h_t) + self.params["b_he"])

        x = T.tensor3("x", dtype=theano.config.floatX)
        h0_enc = T.matrix("h0_enc", dtype=theano.config.floatX)
        ...
        x0 = T.matrix("x0", dtype=theano.config.floatX)
