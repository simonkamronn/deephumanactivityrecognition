...
class GaussianLogDensityLayer(lasagne.layers.MergeLayer):
    ...
    def get_output_for(self, input, **kwargs):
        ...
        if mu.ndim - x.ndim == 2:  # Check for sample dimensions.
            dims = [i for i in range(x.ndim)]
            dims.insert(-1, 'x')
            dims.insert(-1, 'x')
            x = x.dimshuffle(dims)
        elif not mu.ndim == x.ndim:
            raise "Dimension mismatch in density estimation."
...
class BernoulliLogDensityLayer(lasagne.layers.MergeLayer):
    ...
    def get_output_for(self, input, **kwargs):
        ...
        if x_mu.ndim - x.ndim == 2:  # Check for sample dimensions.
            dims = [i for i in range(x.ndim)]
            dims.insert(-1, 'x')
            dims.insert(-1, 'x')
            x = x.dimshuffle(dims)
        elif not x_mu.ndim == x.ndim:
            raise "Dimension mismatch in density estimation."
