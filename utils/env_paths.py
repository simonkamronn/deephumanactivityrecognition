...
def get_project_name():
    ...
    full_path = abspath('.')
    if project_name not in full_path:
        raise "project name must be: %s." % project_name
    return project_name
...
def get_data_path(dataset=""):
    ...
    full_path = abspath('.')
    full_path = full_path[:full_path.find(project_name) + len(project_name)]
...
def get_custom_eval_path(i, root_path):
    ...
    return join(r_path, 'custom_eval_plot_%04d.png' % i)
