...
from lasagne.layers import get_output, get_output_shape, DenseLayer, InputLayer, \
     get_all_params, Conv2DLayer, ConcatLayer, ReshapeLayer
...
class UFCNN(Model):
    def __init__(self, n_in, n_filters, filter_size, n_out, pool_sizes=None, n_hidden=(), downsample=1
        ...
        l2_mask = np.zeros((n_filters, n_filters, filter_size*2+1, 1))
        l2_mask[:, :, 2::2] = 1

        l3_mask = np.zeros((n_filters, n_filters, filter_size*4+1, 1))
        l3_mask[:, :, 4::4] = 1
        ...
        l_h2.W.set_value(np.multiply(l_h2.W.get_value(), l2_mask).astype(theano.config.floatX))
        ...
        l_h3.W.set_value(np.multiply(l_h3.W.get_value(), l3_mask).astype(theano.config.floatX))
        ...
        l_g3.W.set_value(np.multiply(l_g3.W.get_value(), l3_mask).astype(theano.config.floatX))
        ...
        l_g2 = Conv2DLayer(l_h2_g3, num_filters=n_filters, filter_size=(filter_size*2+1, 1),
                           nonlinearity=self.transf, pad='same', name='l_g2')
        l_g2_mask = np.concatenate((l2_mask, l2_mask), axis=1)
        l_g2.W.set_value(np.multiply(l_g2.W.get_value(), l_g2_mask).astype(theano.config.floatX))
        ...
        self.model = DenseLayer(l_g1, num_units=n_out, nonlinearity=out_func)
